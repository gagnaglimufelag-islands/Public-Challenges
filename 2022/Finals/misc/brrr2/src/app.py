...
class Game:
    ...
    def reset(self):
        ...
        self.player_seq = None
        self.deck = None
        self.flag = FLAG if (self.plays == MAX_PLAYS and self.wins >= TARGET_WINS) else None
...
def finish():
    ...
    if seq and len(seq) == 3 and set(seq) <= set('RB'):
        if ''.join(seq) == ''.join(session.computer_seq_str):
            return {'error': 'Same as opponent sequence'}, 400
